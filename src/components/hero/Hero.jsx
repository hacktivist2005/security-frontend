...
import DNSCard from "../DNSCard/DNSCard";
...
import ShodanCard from "../ShodanCard/ShodanCard";
import DomainGraph from "../DomainGraph/DomainGraph";
...
const SectionTitle = ({ icon: Icon, title, color = "text-emerald-400" }) => (
  <div className="flex items-center gap-3 mb-6 border-b border-slate-800/80 pb-3">
    ...
    <h3 className="text-lg font-bold tracking-wide text-slate-100 font-mono uppercase">
      {title}
    </h3>
  </div>
);
...
export default function Hero() {
  ...
  const downloadPDF = async () => {
    if (!result?.whois?.domain && !result?.ip?.query) return;

    setPdfLoading(true);
    try {
      const response = await fetch(
        "https://security-scanner-api-6rct.onrender.com/generate-pdf",
        {
          method: "POST",
